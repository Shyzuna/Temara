...
if (isset($_POST['nomRecherche']))
	$nomRecherche = htmlentities($_POST['nomRecherche']);
else
{
	$numeroRecherche = FicheRechercheTable::nbFiches($compte->mail)+1;
	while (FicheRechercheTable::nomFicheExiste('Recherche '.$numeroRecherche,$compte->mail))
		$numeroRecherche++;
	$nomRecherche = 'Recherche '.$numeroRecherche;
}
...
if (isset($_POST['nomRecherche']) && isset($_POST['budgetMin']) && isset($_POST['budgetMax']) && isset($_POST['surfaceMin']) && isset($_POST['surfaceMax'])
	&& isset($_POST['nbPiecesMin']) && isset($_POST['nbPiecesMax']) && isset($_POST['ville']))
{
	if ($nomRecherche != '' && !(FicheRechercheTable::nomFicheExiste($nomRecherche,$compte->mail))
		&& ($_POST['budgetMin'] == null || is_numeric($_POST['budgetMin'])) && ($_POST['budgetMax'] == null || is_numeric($_POST['budgetMax'])) && ($_POST['budgetMin'] == null || $_POST['budgetMax'] == null || $_POST['budgetMin'] <= $_POST['budgetMax'])
	...
	{
		FicheRechercheTable::addFiche($compte->mail,$nomRecherche,$typeAnnonceTab,$typeBienTab,$prixMin,$prixMax,$surfaceMin,$surfaceMax,$nbPiecesMin,$nbPiecesMax,$ville,$investisseur);
		$_SESSION['ficheCree'] = true;
		header('Location: mes_recherches.php');
	}
	
	// Messages d'erreur
	if ($nomRecherche == '')
		$erreurNomRecherche = '<br /><span class="erreur_form">Veuillez donner un nom à votre recherche.</span>';
	else if (FicheRechercheTable::nomFicheExiste($nomRecherche,$compte->mail))
		$erreurNomRecherche = '<br /><span class="erreur_form">Vous avez déjà donné ce nom à une autre recherche. Veuillez en choisir un autre.</span>';
...
<head>
  ...
  <link href="../Recherche/css/recherche-media-queries.css" rel="stylesheet" type="text/css">
	...
	<div id="corps">
		...
		<div id="ensemble">
			...
			<div class="cadre">
				...
				<h4>Créer une nouvelle fiche de recherche</h4>
